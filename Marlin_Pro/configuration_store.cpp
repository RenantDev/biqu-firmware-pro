...
void printLevelingPara(void)
{
  ...
  for(x=0;x<ABL_GRID_POINTS_X;x++)
  {
    ...
    {
      SERIAL_PROTOCOL_F(bed_level_grid[x][y], 3);
      SERIAL_PROTOCOLPGM(" ");
    }
    SERIAL_PROTOCOLPGM("\r\n");
  }
...
{
  int i=EEPROM_OFFSET+1024;
  char index=0;
  
   _EEPROM_writeData(i,(uint8_t*)&bed_level_grid[0][0],sizeof(bed_level_grid));
...
void readLevelingPara(void)
{
  ...
  char index=0;
   ...
   _EEPROM_readData(i,(uint8_t*)&bed_level_grid[0][0],sizeof(bed_level_grid));
