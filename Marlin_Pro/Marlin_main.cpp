...
//    float z_dest = LOGICAL_Z_POSITION(z_raise);
    float z_dest =current_position[Z_AXIS]+z_raise;
 ...
 */
#if ENABLED(AUTO_BED_LEVELING_BILINEAR)
 void fill_Para_About_Leveling(void)
 {
  if ( 0!= bilinear_grid_spacing[X_AXIS]
    && 0 != bilinear_grid_spacing[Y_AXIS]
    && 0 != bilinear_start[X_AXIS]
    && 0 != bilinear_start[Y_AXIS]
     ) 
  {
   return;
  }
  int left_probe_bed_position = LOGICAL_X_POSITION(LEFT_PROBE_BED_POSITION),
          right_probe_bed_position = LOGICAL_X_POSITION(RIGHT_PROBE_BED_POSITION),
          front_probe_bed_position = LOGICAL_Y_POSITION(FRONT_PROBE_BED_POSITION),
          back_probe_bed_position = LOGICAL_Y_POSITION(BACK_PROBE_BED_POSITION);


      bilinear_grid_spacing[X_AXIS] = (right_probe_bed_position - left_probe_bed_position) / (ABL_GRID_POINTS_X - 1);
      bilinear_grid_spacing[Y_AXIS] = (back_probe_bed_position - front_probe_bed_position) / (ABL_GRID_POINTS_Y - 1);
...
inline void gcode_G28() {
  ...
  if(!planner.abl_enabled)
  {
    char x=0,y=0;
      readLevelingPara();
      for( x=0;x<ABL_GRID_POINTS_X;x++)
      {
        for( y=0;y<ABL_GRID_POINTS_Y;y++)
        {
          if(bed_level_grid[x][y]<100.0&&bed_level_grid[x][y]>-100.0)
          {            
          }
          else
           return;
 ...
 destination[Z_AXIS]=MANUAL_Z_HOME_POS-100;
 ...
 prepare_move_to_destination();
      ...
      for (uint8_t PR_OUTER_VAR = 0; PR_OUTER_VAR < PR_OUTER_END; PR_OUTER_VAR++) {
        ...
        for (int8_t PR_INNER_VAR = inStart; PR_INNER_VAR != inStop; PR_INNER_VAR += inInc) {
          ...
          float xBase = left_probe_bed_position + xGridSpacing * xCount,      //left_probe_bed_position
                yBase = front_probe_bed_position + yGridSpacing * yCount;      //front_probe_bed_position
